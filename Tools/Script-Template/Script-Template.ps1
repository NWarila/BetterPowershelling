...
#region ----------------------------------------------- [Required Functions] -----------------------------------------------------
    ...
    IF ($NoParsing) {
        ...
        @("R","E","F","C","P","N","NE") | ForEach-Object {New-Variable -Force -Name $_ -value ([Int]0)}
        ForEach ($CommandInvocation in $CommandInvocations) {
            ...
            @(@{n="F";v=$FinalCommands.count},@{n="c";v=$CommandInvocation.CommandElements.count},@{n="p";v=0},,@{n="NE";v=([int]1)}) |ForEach-Object {Set-Variable -Name $_.n -Value $_.v}
            ...
            While ($NE+1 -le $C) {
                @(@{n="N";v="PARAM$P"},@{n="R";v=([String]::Empty)},@{n="E";v=([String]::Empty)}) |ForEach-Object {Set-Variable -Name $_.n -Value $_.v}
